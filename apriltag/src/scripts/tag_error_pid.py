...
def control():
    ...
    yaw_pid = PID(70, 8, 2.5, setpoint=0.1)
    x_pid = PID(40, 8, 2.5, setpoint=0.2)
    y_pid = PID(40, 8, 2.5, setpoint=1.2)
    ...
    while not rospy.is_shutdown():
        ...
        if  not aruco_detected and flag_landing == 0:
            ...
            # # else:
            # #     yaw_offset = 1500 + (math.sqrt(abs(yaw)) * 80)
            # yaw_offset = 1500 + np.sign(yaw)*(math.sqrt(abs(yaw)) * 80)

            yaw_pid_control = yaw_pid(yaw)
            x_pid_control = x_pid(x)
            y_pid_control = y_pid(y)

            yaw_offset = 1500 + yaw_pid_control
            x_offset = 1500 + x_pid_control
            y_offset = 1500 +y_pid_control
            ...
            # # else:
            # #     x_offset = 1500 + (math.sqrt(abs(x)) * 50)
            # x_offset = 1500 + np.sign(x)*(math.sqrt(abs(x)) * 50)
            # # if y < 0 :
            # #     y_offset = 1500 + (math.sqrt(abs(y)) * 50)
            # # else:
            # #     y_offset = 1500 + (-math.sqrt(abs(y)) * 50)
            # y_offset = 1500 - np.sign(y)*(math.sqrt(abs(y)) * 50)
